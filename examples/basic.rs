...
use bevy::prelude::*;
use bevy_kira_audio::{AudioApp, AudioPlugin, AudioSource, Channel};
use std::clone::Clone;
...
fn main() {
    App::new()
        ...
        .add_channel::<FirstChannel>()
        .add_channel::<SecondChannel>()
        .add_channel::<ThirdChannel>()
...
fn play_sound_button<T: Component + Default>(
    ...
    mut channel_state: ResMut<ChannelAudioState<T>>,
    audio_handles: Res<AudioHandles>,
    mut interaction_query: Query<(&Interaction, &mut UiColor), With<PlaySingleSound<T>>>,
) {
    for (interaction, mut color) in interaction_query.iter_mut() {
...
#[derive(Component, Default, Clone)]
struct PlaySingleSound<T: Default> {
    _marker: PhantomData<T>,
}
                    ...
                    parent.spawn_bundle(TextBundle {
                        text: Text {
                            sections: vec![TextSection {
                                value: format!("Channel {}", channel_index),
                                style: TextStyle {
                                    font_size: 20.0,
